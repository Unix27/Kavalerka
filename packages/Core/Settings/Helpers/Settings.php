...
class Settings
{
    ...
    public function set($name, $value, $locale = null)
    {
        ...
        if(empty($settingField['locale_based']))
            $locale = null;

        return $this->settingStorage->set($name, $val, $locale);
    }
     ...
     * @param $locale
     */
    public function save($request, $locale)
    {
        // get all defined settings from config
        $allDefinedSettings = $this->getAllSettingFields();

        // set all the fields with updated values
        $allDefinedSettings->each(function ($setting) use ($locale, $request) {

            if(!isset($setting['separator']) && !isset($setting['heading'])) {
                ...
                if (in_array($type, ['file', 'image']) && !isset($setting['mutator'])) {
                    ...
                    $this->set($settingName, $request->get($settingName), $locale);
                }
            }
        });
    ...
    {
        return collect(config('settings.sections', []));
    }
     ...
     *
     * @param $allDefinedSettings Collection
     */
    protected function cleanUpSettings($allDefinedSettings)
    {
        if (!config('settings.remove_abandoned_settings')) {
            return;
        }
    ...
    {
        return config('settings.setting_group');
    }
}
