...
use Localization\Interfaces\LocalizedUrlRoutable;
use Localization\Helpers\LanguageNegotiator;
use Localization\Models\Locale;
use Str;
use Localization\Exceptions\SupportedLocalesNotDefined;
use Localization\Exceptions\UnsupportedLocaleException;
...
class Localization
{
    ...
    public function __construct()
    {
        ...
        // set default locale
        $this->defaultLocale = Locale::where('default', true)->first()->code ?? 'ru';
    ...
    public function setLocale($locale = null)
    {
        ...
        $suffix = $this->configRepository->get('localization.utf8suffix');
        if ($regional) {
     ...
     */
     ...
     public function getCountryIcon($locale)
     {
         $countryCode = $this->getCountryCode($locale);
         if(file_exists(public_path('images/flags/' . $countryCode . '.svg')))
             return asset('images/flags/' . $countryCode . '.svg');
         else return asset('assets/admin/media/svg/icons/Code/Question-circle.svg');
     }

     public function getCountryCode($locale)
     {
         $locale = Locale::where('code', $locale)->first();

         $explode = explode('_', $locale->regional);

         if(isset($explode[1]))
             $countryCode = mb_strtolower($explode[1]);
         else $countryCode = $locale;
    ...
    public function getLocalesMapping()
    {
        if (empty($this->localesMapping)) {
            $this->localesMapping = $this->configRepository->get('localization.localesMapping');
        }
    ...
    public function getSupportedLocales()
    {
        ...
        $localesCollection = Locale::where('active', true)->get();

        foreach ($localesCollection as $item) {
            $locales[$item->code] = [
              'name' =>   $item->name,
              'script' =>   $item->script,
              'native' =>   $item->native,
    ...
    {
        $localesCollection = Locale::where('published', true)->get();
    ...
    public function getLocalesOrder()
    {
        ...
        $order = $this->configRepository->get('localization.localesOrder');
